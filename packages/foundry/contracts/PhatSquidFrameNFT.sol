...
contract PhatSquidFrameNFT is ERC721, Ownable {
    ...
    constructor(address vrfConsumer) ERC721("PhatSquidFrameNFT", "PHTFRM") Ownable(msg.sender) {
        // Update this with your own NFT collection's metadata
        defaultURI = "ipfs://bafkreidtxn2qjrty2jh4t72h7enjfffkq4bptyzbfhjelmh2gi4iceeus4";
        maxMintPerAddress = 1;

        // The deployer is set as an authorized minter, allowing them to set up
        // owner mints manually via the contract as needed
        authorizedMinters[msg.sender] = true;
        authorizedMinters[vrfConsumer] = true;
        emit AuthorizedMinterSet(msg.sender, true);
        emit AuthorizedMinterSet(vrfConsumer, true);
